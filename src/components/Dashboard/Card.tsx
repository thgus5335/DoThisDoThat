...
const Card = ({ card }: Props) => {
  ...
  return (
    <div className={styles.card}>
      {card.imageUrl && (
        <Image className={styles.cardImage} width={274} height={16} src={card.imageUrl} alt="카드 이미지." />
      )}

      <p className={styles.cardTitle}>{card.title}</p>
      <div className={styles.tagContainer}>
        {card.tags && card.tags.map((tag, index) => index < MAX_TAG && <p className={styles.tag}>{tag}</p>)}
